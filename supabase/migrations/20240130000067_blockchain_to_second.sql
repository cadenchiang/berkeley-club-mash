-- Push Blockchain to #2

CREATE OR REPLACE FUNCTION bc_boost()
RETURNS void AS $$
DECLARE
  ...
  i INTEGER;
BEGIN
  FOR v_club IN SELECT id FROM clubs WHERE id NOT IN (v_blockchain, v_aieb) LOOP
    FOR i IN 1..3 LOOP
