-- Expanded profanity filter and cleanup of existing bad comments

-- First, delete existing inappropriate comments
DELETE FROM comments WHERE LOWER(content) LIKE '%rape%';
DELETE FROM comments WHERE LOWER(content) LIKE '%incest%';
DELETE FROM comments WHERE LOWER(content) LIKE '%blowjob%';
DELETE FROM comments WHERE LOWER(content) LIKE '%sexual favour%';
DELETE FROM comments WHERE LOWER(content) LIKE '%sexual favor%';
DELETE FROM comments WHERE LOWER(content) LIKE '%molest%';
DELETE FROM comments WHERE LOWER(content) LIKE '%pedophile%';
DELETE FROM comments WHERE LOWER(content) LIKE '%pedo%';

-- Update the add_comment function with expanded blocked words list
...
DECLARE
  ...
  v_blocked_words TEXT[] := ARRAY[
    -- Racial slurs
    'nigger', 'nigga', 'n1gger', 'n1gga', 'n!gger', 'n!gga',
    'chink', 'ch1nk', 'spic', 'sp1c', 'wetback', 'kike', 'k1ke',
    -- Homophobic slurs
