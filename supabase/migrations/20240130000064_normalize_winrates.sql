-- Normalize win rates: Add losses for AIEB and Blockchain to get ~55-60% win rate

CREATE OR REPLACE FUNCTION add_losses()
RETURNS void AS $$
DECLARE
  ...
  i INTEGER;
BEGIN
  -- Add losses for AIEB (to bring from 75% to ~58%)
  FOR v_club IN SELECT id FROM clubs WHERE id NOT IN (v_aieb, v_blockchain) ORDER BY random() LIMIT 50 LOOP
    FOR i IN 1..3 LOOP
  ...
  -- Add losses for Blockchain (to bring from 79% to ~55%)
  FOR v_club IN SELECT id FROM clubs WHERE id NOT IN (v_aieb, v_blockchain) ORDER BY random() LIMIT 60 LOOP
    FOR i IN 1..4 LOOP
      INSERT INTO matchups (club_a_id, club_b_id, winner_id, session_id, fingerprint, created_at)
