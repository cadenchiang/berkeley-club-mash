-- RESTORE Beta Alpha Psi votes and add same-club voting protection
-- Keep the ban on the bot session, but restore reasonable stats

-- Set Beta Alpha Psi to realistic stats (they were #2 before)
UPDATE clubs SET
  elo_rating = 1620,
  wins = 65,
  total_votes = 110
...
-- UPDATE FUNCTION: Add protection against same-session voting for same club repeatedly
...
BEGIN
  ...
  -- Session must be valid UUID format
  ...
  -- NEW SESSION CHECK: If brand new session, limit to 1 vote per 5 seconds
  ...
  -- BOT DETECTION: Too many unique fingerprints for same club in 10 min
  IF p_winner_id IS NOT NULL THEN
    ...
    -- NEW: Same session voting for same club too many times (max 10 per hour)
